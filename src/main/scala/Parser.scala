            ...
            eq match
                ...
                case RecEq(name, e:Array[Equation], ns) =>
                    ...
                    if op_cand.length == 1 then
                                ...
                                sep match
                                    ...
                                    case _ =>
                                        ...
                                        if !(ops contains sep.operation) || !(tt equals zeqp._1) then
                                            throw new RuntimeException("Could not match \"" + sep.operation + "\" (with type: \"" + tt + "\") with expected type \"" + zeqp._1 + "\" in operation call for \"" + name + "\"")
                                        sep
                        ...
                        if actual_cand.length > 1 then throw new TypeException("Ambiguous operation usage for operation \"" + name + "\"")
                        ...
                        val pass2pars = (pass1pars zip actual.par) map (x => x._1 match
                            ...
                            case nsep:RecEq => 
                                if nsep.ref_op.get.ret != x._2 then
                                    new TypeException("Could not match \"" + nsep.operation + "\" with expected type \"" + x._2 + "\" in operation call for \"" + name + "\"")
                                x._1
                        )
